...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.avgpool(x4)
        x_global_pool = x.view(x.size(0), -1)
        x_fc = self.fc(x_global_pool)

        # self.embeddings = nn.Sequential(OrderedDict([('global_pool_layer', x_global_pool)]))
...
def resnet34(model_params):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
        model_params (dict): Parameters for the model (optional)
    """
