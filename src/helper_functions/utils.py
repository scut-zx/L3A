...
def calculate_metrics(preds, targets, thre):
    ...
    # 将模型预测值转换为二值化的预测值，大于阈值的为1，小于阈值的为0
...
def calculate_metrics_sk(preds, targets, thre):

    # 将模型预测值转换为二值化的预测值，大于阈值的为1，小于阈值的为0
    prediction = preds.gt(thre).long()
    count = targets.size(0)

    precision_c = precision_score(targets.cpu(), prediction.cpu(), average=None) * 100.0
    recall_c = recall_score(targets.cpu(), prediction.cpu(), average=None) * 100.0
    f1_c = f1_score(targets.cpu(), prediction.cpu(), average=None) * 100.0

    df = pd.DataFrame(
        [f1_c], columns=[f"score_{i}" for i in range(1, len(f1_c) + 1)]
    )
    # 保存到 CSV 文件，如果文件已经存在，那么在末尾添加新的数据
    # df.to_csv("f1_c_true_de_nor_10+10_525.csv", mode="a", header=False, index=False)

    mean_p_c = precision_c.mean()
    mean_r_c = recall_c.mean()
    mean_f_c = f1_c.mean()

    precision_o = precision_score(targets.cpu(), prediction.cpu(), average='micro') * 100.0
    recall_o = recall_score(targets.cpu(), prediction.cpu(), average='micro') * 100.0
    f1_o = f1_score(targets.cpu(), prediction.cpu(), average='micro') * 100.0
