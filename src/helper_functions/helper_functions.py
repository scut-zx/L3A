...
# weighting mechanism
def normalize_sqrt(freqs: list, beta=0.5):
    inv_freqs = 1 / np.array(freqs)
    inv_freqs[np.isinf(inv_freqs)] = 0
    inv_freqs = np.power(inv_freqs, beta)    
    normalized_weights = len(inv_freqs) * inv_freqs / sum(inv_freqs)
