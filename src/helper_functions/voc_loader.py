...
class VOC(Loader):
    def __init__(self, year, proposals, split, num_proposals=2000, included=[],
                 cats=VOC_CATS, root='/data1/zhangx/VOCdevkit', transform=defaultTransform):
        super().__init__()
        assert year in ['07', '12']
        self.CLASSES = VOC_CATS
        self.dataset = 'voc'
        self.year = year
        self.root = root + ('/VOC20%s/' % year)
        self.split = split
...
if __name__ == '__main__':
    loader = VOC('07', 'edgeboxes', 'trainval', included=range(16,20))

    img_file = []
    for nextclass in range(0,20):
    ...
    for i in range(len(img_file)):
        for j in range(i+1, len(img_file)):
            print(img_file[i]&img_file[j])
